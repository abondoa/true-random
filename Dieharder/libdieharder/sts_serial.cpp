...
int sts_serial(Test **test,int irun)
{
 ...
 for(m = 1;m < nb1;m++) {
   freq[m] = (double *)malloc(pow((double)2,(int)m)*sizeof(double));
   memset(freq[m],0,pow((double)2,(int)m)*sizeof(double));
 }
 ...
 for(m=1;m<nb1;m++){
   ...
   psi2[m] = pow((double)2,(int)m)*psi2[m]/bsize - bsize;
   MYDEBUG(D_STS_SERIAL){
 ...
 for(m=2;m<nb1;m++){
   ...
   pvalue = gsl_sf_gamma_inc_Q(pow((double)2,(int)m-2),delpsi2[m]/2.0);
   if(irun == 0){
     test[j]->ntuple = m;
   }
   ...
   if(m>2){
     pvalue = gsl_sf_gamma_inc_Q(pow((double)2,(int)m-3),del2psi2[m]/2.0);
     if(irun == 0){
       test[j]->ntuple = m;
     }
     test[j++]->pvalues[irun] = pvalue;
